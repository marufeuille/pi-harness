...
import { linearIssueIdFromTicket, loadTicket } from "./ticket.ts";
...
test("Markdown ticket keeps its heading title and body", async () => {
  ...
  try {
    ...
    assert.equal(linearIssueIdFromTicket({ path: file, title: "Regression title", body: "" }), undefined);
    assert.equal(linearIssueIdFromTicket({ path: "linear:ABC-1", title: "t", body: "b" }), "ABC-1");
  } finally {
