...
Linear (`--linear`) 実行では、停止中も In Progress のままです。再開時は `--linear` を再指定しなくても元課題の識別情報を引き継ぎ、開始状態を書き直さず、`ready` または `production-ok` のときだけ元課題を Done にします。異なる課題の `--linear` は作業開始前に拒否します。返却・エスカレーション・失敗時は Done にせず、状態を巻き戻しません。
...
Linear から取り込むのは指定した課題 1 件のタイトルと本文のみです。`--linear` 実行では開始前に In Progress、ワークフローが `ready` または `production-ok` で終わった後に Done へ更新します。再開では保存した課題識別情報を使い、`--linear` の再指定は不要です。別の課題を指定した場合は作業開始前に拒否します。状態更新に失敗した場合は理由を表示して非ゼロ終了し、開始更新に失敗した場合はワークフローを開始しません。返却・エスカレーション・失敗時は Done にせず、状態を巻き戻しません。Markdown (`--ticket`) 実行では Linear の状態を更新しません。課題から実装手順を補完したり、マージ後に Linear へ起票したりする機能は対象外です。
